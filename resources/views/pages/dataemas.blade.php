...
@section('contents')
  <div class="container">
    <div class="row">
      ...
      <a class="waves-effect waves-light btn blue"href="{{ route('rlb.index') }}">
        Hitung Persamaan
      </a>
      <!-- end modal trigger -->
      ...
      <div class="_data-table-overflow-x _push-t-d">
        <table id="tblDataEmas" class="display" style="width:100%">
          @csrf
          <thead>
            <tr>
              <th>#</th>
        ...
        <div class="modal-content">
          ...
          <form id="frmDataemas">
            ...
            <div class="input-field">
              ...
              <span class="txtHarga helper-text red-text"></span>
            </div>
            <div class="input-field">
      ...
      <!-- Modal update -->
      <div id="updateDataemas" class="modal modal-fixed-footer">
        <div class="modal-content">
          <h4 class="modalTitle">Tambah Data</h4>
          <form id="frmUpdateDataemas">
            ...
            <div class="input-field">
              <input name="hargaemas" id="hargaemas" type="number" class="validate" placeholder="(Ex: 500000)">
              <label for="hargaemas">Harga Emas (Rp)</label>
              <span class="editHarga helper-text red-text"></span>
            </div>
            <div class="input-field">
...
@push('scripts')
  <script type="text/javascript">
    ...
    $('document').ready(function(){
      ...
      table = $('#tblDataEmas').DataTable({ 
        stateSave: true,
        responsive: true,
        processing: true,
        serverSide : true,
        // order : [0,'desc'],
        ajax : '{{ route('data.emas') }}',
        columns: [
          { data: 'DT_RowIndex', name : 'id' },
          { data: 'bulan' , name : 'bulan' },
          { data: 'hargaemas' , name : 'hargaemas' },
    ...
    $('#addData').click(function(e){
      ...
      var frm = $('#frmDataemas');
      $.ajax({
        url : '{{ route('dataemas.store') }}',
        type : 'POST',
        dataType: 'json',
        data : {
          'csrf-token': $('input[name=_token]').val(), 
           bulan : $('[name=bulan]').val(),
           hargaemas : $('[name=hargaemas]').val(),
        ...
        success:function(data){
          ...
          if (data.errors) {
            ...
            if (data.errors.hargaemas) { //if valisate harga emas error
              $('.txtHarga').text(data.errors.hargaemas);
              $('.txtHarga').show();
            }
    ...
    $('#tblDataEmas').on('click','.btnEdit[data-edit]',function(e){
      e.preventDefault();
    ...
    $('#updateData').on('click',function(e){
      e.preventDefault();
      var url = "dataemas/" + $('[name=id]').val();
      var frm = $('#frmUpdateDataemas');
      $.ajax({
        type :'PUT',
        url : url,
        dataType : 'json',
        ...
        success:function(data){
          ...
          if (data.errors) {
            ...
            if (data.errors.hargaemas) { //if valisate harga emas error
              $('.editHarga').text(data.errors.hargaemas);
              $('.editHarga').show();
            }
            if (data.errors.inflasi) { //if inflasi error
    ...
    //deleting data
    $('#tblDataEmas').on('click','.btnDelete[data-remove]', function(e){
      e.preventDefault();
